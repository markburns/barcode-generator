class BarcodeDependencyChecker
  ...
  def self.check_image_magick
    ...
    if convert_filename.blank?
      raise ImageMagickMissingException, "You must have imagemagick installed to generate barcodes"
    end
  end
  ...
  def self.check_ghost_script
    ...
    if gs_filename.blank?
      raise GhostScriptMissingException, "You must have ghostscript installed to generate barcodes"
    end
  end
